...
**必須識別特徴（最優先）:**
- ✅ **上部中央に黒い切り抜き（ノッチ）または黒い楕円形（ダイナミックアイランド）が存在**
...
3. **斜め向き（Diagonal/Tilted）**:
   ...
   - **判定方法**:
     ...
     - ベクトルの角度により主要な向きを判定:
       ...
       - 45°付近 → 斜め上向き（↗）
       - 135°付近 → 斜め横向き（↘）
   - **重要**: 斜め向きでもスマートフォンとして識別（ノッチの存在が最優先）
   - 縦方向矢印: **↗（斜め上）** または **↘（斜め横）**
...
**注意**: アスペクト比は長方形（正面ビュー）の場合のみ有効です。平行四辺形や台形の場合、見かけのアスペクト比は実際と異なるため参考値とします。
