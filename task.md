...
- [ ] `src/constants/deviceColors.ts` を作成
  ```typescript
  ...
  ```
- [ ] 各デバイスに対応する色を自動割り当て
- [ ] オーバーレイ描画時にデバイスごとの色を適用

#### 1-3. 複数画像アップロード対応
- [ ] アップロード画像を配列で管理（最大3枚）
- [ ] 各デバイスに対応する画像を個別に設定可能に
...
### 6. Canvas API透視変換の実装
**目的**: より自然な画面合成のための高度な変換

#### 6-1. 透視変換ユーティリティの作成
- [ ] `src/utils/perspectiveTransform.ts` を作成
- [ ] 4点透視変換行列の計算実装
- [ ] 微小三角形分割による高品質描画

#### 6-2. 既存の合成処理への統合
- [ ] cover/containモードに加えて透視変換モード追加
- [ ] 角度のある画面への対応
- [ ] テキスト明瞭性の最適化
